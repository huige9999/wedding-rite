...
<template>
  <div class="container">
    ...
    <div class="main">
      ...
      <WorkShowcase
        ...
        :work-list="workList"
        is-share-workmate
        :user-id="shareUserInfo?.id"
      />
      <!-- 分割线 -->
      <div class="hr second" />
      <!-- 动态 -->
      <ActivityShowcase
        v-if="activityList.length > 0 && shareUserInfo.isShowVideoModel"
  ...
  <MyPreviewShareAboutMePopup
    ...
    :age="shareUserInfo.age"
    :height="shareUserInfo.height"
    :weight="shareUserInfo.weight"
...
import { recordInvite } from '@/api/user-api';
import { reqGetWorkList } from '@/api/work-api';
import PhotoSwiper from '@/components/PhotoSwiper.vue';
import shareWorkmateBottom from '@/components/share-workmate-bottom.vue';
...
import useInvitor from '@/stores/invitor-store';
import { useSchedule } from '@/stores/schedule-store';
import useShareWorkMate from '@/stores/share-workmate';
import useUser from '@/stores/user-store';
import { navigateTo } from '@/utils/navigate';
import { parseActivityList,
  parseWorkList } from '@/utils/parse';
import { splitStrByLineBreak } from '@/utils/tools';

const ctx = getCurrentInstance();

const shareWorkMateStore = useShareWorkMate();
const { getUserInfoById } = shareWorkMateStore;
const {
  shareUserInfo,
  personalInfo,
} = storeToRefs(shareWorkMateStore);

const userStore = useUser();
const { updateInviteWorkMateID } = userStore;
const { inviteWorkMateID } = storeToRefs(userStore);

const scheduleStore = useSchedule();
const { getScheduleListAll, getSchedulePosterData } = scheduleStore;

const invitorStore = useInvitor();
const { invitorUserId } = storeToRefs(invitorStore);
...
onShareAppMessage(() => ({
  ...
  path: `/subpkg/wedding-link/invite-workmate-preview-share?userId=${invitorUserId.value}&inviterId=${invitorUserId.value}`,
  imageUrl: personalInfo?.value?.photoList?.[0],
}));

// #endregion

// #region 我的用户信息和个人信息相关
...
onLoad((options) => {
  ...
  /** 更新邀请人的id */
  updateInviteWorkMateID(options.userId);
  // 判断是否登录且不等于自己的id，登录了直接记录邀请
  const isLogin = uni.getStorageSync('isFormalToken');
  if (isLogin && inviteWorkMateID.value && options.userId !== inviteWorkMateID.value) {
    recordInvite(options.userId);
  }

  getUserInfoById(options.userId);
  // 根据userId获取档期列表
  // XXX 截止2023年7月20日，档期数据最多也不到100条，所以这里不分页获取，后续有需要再改
  getScheduleListAll(options.userId);
  getSchedulePosterData(options.userId);
});
...
onLoad((options) => {
  if (options?.userId) {
    getActivityList(options.userId);
  }
});
// #endregion
