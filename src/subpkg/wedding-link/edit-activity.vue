...
import { reqGetActivityDetail, reqEditActivity } from '@/api/activity-api';
...
onLoad(async (option) => {
  ...
  const resData = (await reqGetActivityDetail(activityId))?.data as ResData<ActivityResInfo>;

  if (resData.code !== 200) {
    showToast('获取动态信息失败，请稍后重试');
    back();
    return;
  }
  const mediaInfo = JSON.parse(resData.data.mediaInfo) as MediaInfo;
  activityType.value = mediaInfo.type;
  const videos = activityType.value === 'video' ? mediaInfo.mediaPath : [];
  const images = activityType.value === 'image' ? mediaInfo.mediaPath : [];
...
const addBtnClick = async () => {
  ...
  // 如果有超过100M的文件，即有没添加的文件，则提示用户
  if (tempFiles.some((item) => item.size >= 100 * 1024 * 1024)) {
    showToast('文件大小不能超过100M');
  }
...
/**
 * 将列表中的本地图片/视频上传到服务器，返回上传后的图片/视频地址
 * @param list 图片/视频列表
 * @returns 上传后的图片/视频地址
 */
const uploadMedia = async (list: string[]) => {
  const uploadRes = await Promise.all(
    // 只有本地图片/视频才需要上传
    list
      .filter((item) => item.startsWith('http://tmp'))
      .map((item) => reqUploadAnyFile(item)),
  ...
  // 上传成功，替换原来的本地图片/视频地址，返回新的图片/视频地址
  return list
    .filter((item) => !item.startsWith('http://tmp'))
    .concat(uploadRes.map((item) => parseUploadFileUrl(item.data)));
};
...
const finishBtnClick = async () => {
  ...
  console.log('type', activityType.value);
  // 根据动态类型，上传图片/视频
  const { images, videos } = activityInfo.value;
  ...
  // 上传图片
  if (activityType.value === 'image') {
    newActivityInfo.images = await uploadMedia(images);
    console.log('newImages', newActivityInfo.images);
    [activityInfo.value.cover] = newActivityInfo.images;
  // 上传视频
  } else {
    newActivityInfo.videos = await uploadMedia(videos);
    console.log('newVideos', newActivityInfo.videos);
    activityInfo.value.cover = `${newActivityInfo.videos[0]}?x-oss-process=video/snapshot,t_0,f_jpg`;
  }

  console.log('activityInfo', activityInfo.value);
  const sendRes = await reqEditActivity(newActivityInfo);
  console.log('sendRes', sendRes);
  if ((sendRes?.data as ResData<AnyObject>).code !== 200) {
    showToast('编辑失败，请稍后再试');
  // 发布成功
  } else {
    showToast('编辑成功');
