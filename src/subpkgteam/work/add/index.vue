  ...
  <div class="container">
    ...
    <div class="form">
      <div class="cover" @click="uploadCover">
        <div class="upload-tip" v-if="!formData.cover">
          添加封面，推荐比例（16:9）
        </div>
...
import { reqAddWork, reqGetWorksDetail, reqEditWorks } from "@/api/team-api";
  ...
  try {
    ...
    if(workId.value) {
      const res = await reqEditWorks({
        ...formData,
        find_id: workId.value,
      });
      console.log(res);
    } else {
      const res = await reqAddWork(formData);
      console.log(res);
    }
